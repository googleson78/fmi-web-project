...
Чрез `serve api counter` получаваме подходящо представяне (`Application`), което да дадем на `warp`.
```haskell
startServer :: IO ()
startServer = do
...
На нас ни трябват двете отделни функции, затова поотделно ги свързваме с имена.
